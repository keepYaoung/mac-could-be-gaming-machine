...
### 첫 실행 온보딩 — 언어 선택 → 자동 프로비저닝 (실측 발굴, 2026-06-30)

원클릭 설치 패키지를 지향하므로 **온보딩에 "언어 선택" 단계**가 필요하다. 비라틴 언어
(한국어/CJK 등) 사용자는 Wine prefix에 해당 폰트가 없으면 **Steam·런처·게임 UI 텍스트가
□□□로 깨진다** (게임 3D 렌더와 무관 — 순수 prefix 폰트 문제). 따라서:

- 온보딩에서 **사용자 언어를 1회 선택** → prefix에 **CJK 폰트(winetricks `corefonts`+`cjkfonts`)
  + locale 자동 설치**. 사용자는 winetricks·폰트 존재를 몰라도 됨(무전문용어 위저드와 정합).
- 프로파일/래퍼 생성 시 1회 수행. 게임별 언어 override는 §6 프로파일에서.
- **발굴 경위**: RDR2 PoC에서 한글이 전부 깨짐 → 누락 CJK 폰트가 원인. 이런 마찰을 자동
  해소하는 게 "Sikarugir 재포장이 아닌" 차별점의 구체적 예.
